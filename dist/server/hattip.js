...
const Fragment$1 = jsxRuntime.Fragment;
const jsx = jsxRuntime.jsx;
const jsxs = jsxRuntime.jsxs;
const convex = new ConvexReactClient("https://elegant-meerkat-890.convex.cloud");
...
const hooks = {
  ...
  wrapApp(app) {
    return /* @__PURE__ */ jsx(ConvexClientProvider, { children: app });
  },
...
async function renderPageRoute(ctx) {
  ...
  const routes = (await import("./assets/virtual_rakkasjs_server-page-routes-4b208df0.js")).default;
  let {
    url: { pathname }
  } = ctx;
  ...
  if (!pageContext) {
    ...
    await ((_b = hooks.extendPageContext) == null ? void 0 : _b.call(hooks, pageContext));
    pageContextMap.set(ctx.request, pageContext);
  }
  let found;
  const beforePageLookupHandlers = [hooks.beforePageLookup].filter(Boolean);
  if (ctx.notFound) {
    do {
      if (!pathname.endsWith("/")) {
        pathname += "/";
  ...
  for (const hooks2 of reversePageHooks) {
    if (hooks2 == null ? void 0 : hooks2.wrapApp) {
      app = hooks2.wrapApp(app);
    }
  }
  ...
  for (const hooks2 of pageHooks) {
    if (hooks2 == null ? void 0 : hooks2.wrapSsrStream) {
      wrapperStream = hooks2.wrapSsrStream(wrapperStream);
    }
  }
  ...
  async function pipe() {
    ...
    for await (const chunk of wrapperStream) {
      for (const hooks2 of pageHooks) {
        if (hooks2 == null ? void 0 : hooks2.emitBeforeSsrChunk) {
          const text = hooks2.emitBeforeSsrChunk();
          if (text) {
...
function renderHead(prefetchOutput, renderMode, actionData = void 0, actionErrorIndex = -1, pageHooks = []) {
  ...
  for (const hooks2 of pageHooks) {
    if (hooks2 == null ? void 0 : hooks2.emitToDocumentHead) {
      result += hooks2.emitToDocumentHead();
    }
  }
  result += `</head><body>`;
  return result;
...
var runServerSideServerHooks = {
  middleware: {
    beforeApiRoutes: async (ctx) => {
      ...
      const manifest = await import("./assets/virtual_rakkasjs_run-server-side_manifest-f027b769.js");
...
function createRequestHandler(userHooks = {}) {
  const hooks2 = [...feature_server_hooks_default, userHooks];
  return compose(
    [
