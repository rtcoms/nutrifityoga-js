...
// ../../node_modules/.pnpm/@brillout+json-serializer@0.5.3/node_modules/@brillout/json-serializer/dist/cjs/parse.js
...
async function loadRoute(pageContext, lastFound, try404, beforePageLookupHandlers, actionData, ssrMeta, ssrPreloaded, ssrModules) {
  ...
  const promises = importers.map(
    async (importer, i2) => Promise.resolve(ssrModules?.[importers.length - 1 - i2] || importer()).then(
      async (module) => {
        const preload = import.meta.hot && updatedComponents ? updatedComponents[i2]?.preload : module.default?.preload;
        try {
          if (!import.meta.env.SSR && i2 === window.$RAKKAS_ACTION_ERROR_INDEX) {
            throw new Error("Action error");
            delete window.$RAKKAS_ACTION_ERROR_INDEX;
          }
          const preloaded = ssrPreloaded?.[i2] ?? await preload?.(preloadContext);
  ...
  if (import.meta.env.SSR) {
    ...
    meta = {};
    preloaded.forEach((p2) => Object.assign(meta, p2?.meta));
    preloadNode = preloaded.map(
      (result, i2) => (result?.head || result?.redirect) && /* @__PURE__ */ React9.createElement(Fragment, { key: i2 }, result?.head, result?.redirect && /* @__PURE__ */ React9.createElement(Redirect, { ...result?.redirect }))
    ).filter(Boolean);
  }
...
async function renderPageRoute(ctx) {
  ...
  const preloaded = await Promise.all(
    reverseModules.map(async (m2, i2) => {
      try {
        ...
        const preloaded2 = await m2.default?.preload?.(preloadContext);
        return preloaded2;
      } catch (preloadError) {
        modules[i2] = {
          default() {
  ...
  const preloadNode = preloaded.map((result, i2) => {
    return (result?.head || result?.redirect) && /* @__PURE__ */ React10.createElement(Fragment2, { key: i2 }, result?.head, result?.redirect && /* @__PURE__ */ React10.createElement(Redirect, { ...result?.redirect }));
  }).filter(Boolean);
  let app = /* @__PURE__ */ React10.createElement(
    App,
    {
  ...
  app = /* @__PURE__ */ React10.createElement(ServerSideContext.Provider, { value: ctx }, /* @__PURE__ */ React10.createElement(IsomorphicContext.Provider, { value: pageContext }, app));
  ...
  for (const m2 of modules) {
    const headers2 = await m2.headers?.(preloadContext, meta);
    if (headers2) {
    ...
    if (process.env.RAKKAS_PRERENDER === "true") {
      ...
      for (const m3 of modules) {
        const value = await m3.prerender?.(preloadContext, meta);
        if (value) {
          prerender2 = {
            ...prerender2,
            ...value,
  ...
  app = /* @__PURE__ */ React10.createElement("div", { id: "root" }, /* @__PURE__ */ React10.createElement(ResponseContext.Provider, { value: updateHeaders }, /* @__PURE__ */ React10.createElement(
...
function renderHead(prefetchOutput, renderMode, actionData = void 0, actionErrorIndex = -1, pageHooks = []) {
  ...
  (actionData === void 0 && renderMode !== "server" ? "" : `<script>$RAKKAS_ACTION_DATA=${uneval2(actionData)}</script>`);
...
var headServerHooks = {
  createPageHooks() {
    ...
    return {
      wrapApp: (app) => {
        return /* @__PURE__ */ React11.createElement(q, { context: helmetContext }, app);
      },
      emitToDocumentHead() {
...
var useQueryServerHooks = {
  createPageHooks() {
    ...
    return {
      wrapApp: (app) => {
        return /* @__PURE__ */ React12.createElement(QueryCacheContext.Provider, { value: cache2 }, app);
      },
      extendPageContext(ctx) {
      ...
      if (ctx.request.headers.get("accept") === EventStreamContentType && result instanceof ReadableStream) {
        const { readable, writable } = new TransformStream({
          transform(chunk, controller) {
            controller.enqueue(`data: ${uneval4(chunk)}
  ...
  middleware: {
    beforePages: (ctx) => {
      ctx.fetch = async (input, init2) => {
        ...
        const credentials = requestCredentials ?? init2?.credentials ?? "same-origin";
...
var clientSideNavigationServerHooks = {
  createPageHooks(ctx) {
    return {
      wrapApp(app) {
        return /* @__PURE__ */ React13.createElement(LocationContext.Provider, { value: ctx.url.href }, app);
      }
    };
  }
};
