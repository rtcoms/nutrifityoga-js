        ...
        exit(program) {
          ...
          program.traverse({
            ExportNamedDeclaration: {
              enter(path5) {
                if (t2.isFunctionDeclaration(path5.node.declaration) && SSR_EXPORTS.includes(path5.node.declaration.id.name)) {
                  path5.remove();
                  modified = true;
                } else if (t2.isVariableDeclaration(path5.node.declaration)) {
                  const declarations = path5.get("declaration").get("declarations");
                ...
                } else if (path5.node.specifiers.length) {
...
function fsRoutes() {
  ...
  const isRouteFile = (filename) => {
    ...
    return filename.startsWith(path4.posix.join(root, "src/routes/")) && (isDottedRouteFile(filename) || isUndottedRouteFile(filename)) && !isCssFile(filename);
  };
  let routesChanged;
  const result = {
      ...
      rakkas: {
        async getRoutes() {
          ...
          let files = await glob(
            ...
            { cwd: path4.join(root, "src/routes") }
          );
          if (path4.sep !== "/") {
            files = files.map((x) => x.replaceAll(path4.sep, "/"));
          }
          ...
          return [
            ...pages2.map((page) => {
              const path5 = filenameToPathPattern(page);
              return {
                type: "page",
                path: path5,
...
function wraps(wrapper, target) {
  const wrapperDir = path4.dirname(wrapper);
...
function toPosix(name) {
  if (path4.sep !== "/") {
    return name.replaceAll(path4.sep, "/");
  }
  return name;
}
var cacheBuster = 0;
...
function sortRoutes(routes2) {
  const processedRoutes1 = routes2.map((route) => ({
    original: route,
  ...
  return {
    ...
    async load(id) {
      if (id === "\0virtual:rakkasjs:server-page-routes") {
        return generateServerPageRoutesModule(routes2);
